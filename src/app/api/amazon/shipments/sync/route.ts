...
import { syncShipmentsToNetSuite } from "@/lib/netsuite-invoices";

export const dynamic = "force-dynamic";

/**
 * POST /api/amazon/shipments/sync
 * Sync a saved shipment report to NetSuite as Invoices.
...
export async function POST(req: Request) {
  try {
    ...
    // Run sync
    const shipments = report.shipments as any[];
    const result = await syncShipmentsToNetSuite(orgId, shipments, syncJob.id);
    ...
    return NextResponse.json(
      { error: err.message || "Failed to sync shipments" },
      { status: 500 }
    );
  }
}
